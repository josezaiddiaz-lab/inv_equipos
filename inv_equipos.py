...
with st.sidebar.form("form_nuevo", clear_on_submit=True):
  ...
  if submit:
    nuevo_registro = pd.DataFrame({
        "Cliente": [c_cliente.upper()],  # Solo cliente en mayúsculas
...
def generar_pdf(dataframe_filtrado, estado_filtro):
  buffer = io.BytesIO()
  # Configurar hoja horizontal (landscape) para que entren todas las columnas bien
  ...
  # Seleccionar columnas importantes para el PDF
  columnas_mostrar = [
      "Cliente",
      "Tipo de equipo",
      "Serie",
  ...
  # Convertir filas del DataFrame a listas para ReportLab, usando Paragraph para ajuste de texto automático
  ...
  # Tamaños de columnas ajustados al ancho horizontal de la página
...
# Botón de descarga en PDF debajo de la tabla
    ...
    with st.form("form_editar"):
      ...
      if actualizar:
        df.loc[index_a_editar] = [
            e_cliente.upper(),  # Cliente en mayúsculas
            e_tipo,
            e_serie,
            e_marca,
            e_modelo,
