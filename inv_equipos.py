  ...
  try:
    ...
    if df.empty:
      return pd.DataFrame(columns=[
          "Cliente", "Departamento", "Tipo de equipo", "Serie", "Marca", "Modelo", 
          "Diagnóstico", "Observaciones", "Fecha ingreso", "Fecha salida", "Estado"
      ])
    df.columns = df.columns.astype(str).str.strip()
    
    # Asegurar que la columna Departamento exista si el sheet es antiguo
    if "Departamento" not in df.columns:
      df["Departamento"] = ""
    
    # Convertir todas las columnas a texto para evitar conflictos de tipos
    df = df.astype(str)
    df = df.replace("nan", "")
...
# Función para resaltar filas pendientes en el color rojo oscuro de la captura (#4a1515 o similar)
def resaltar_pendientes(row):
  # Comprobamos si la columna 'Estado' dice Pendiente (ignorando mayúsculas/minúsculas)
  if str(row.get("Estado", "")).strip().capitalize() == "Pendiente":
    return ['background-color: #581818; color: #ffffff'] * len(row)
  return [''] * len(row)
...
# Opción adicional en la barra lateral: Generar Orden de Servicio múltiple
st.sidebar.markdown("---")
st.sidebar.header("📄 Generador de Órdenes")
modo_orden_servicio = st.sidebar.checkbox("📑 Ir a Orden de Servicio por selección")
...
with st.sidebar.form("form_nuevo", clear_on_submit=True):
  ...
  c_departamento = st.text_input("Departamento")
  ...
  if submit:
    nuevo_registro = pd.DataFrame({
        ...
        "Departamento": [c_departamento.upper()],
...
# --- SI ESTÁ ACTIVO EL MODO ORDEN DE SERVICIO ---
if modo_orden_servicio:
  st.subheader("📄 Generación de Orden de Servicio por Selección Múltiple")
  st.markdown("Filtra los equipos y selecciona los que deseas incluir en la orden de servicio.")

  col1, col2, col3 = st.columns(3)
  with col1:
    f_os_cliente = st.text_input("Buscar por Cliente (OS)")
  with col2:
    f_os_estado = st.selectbox("Filtrar por Estado (OS)", ["Todos", "Pendiente", "Reparado"])
  with col3:
    f_os_serie = st.text_input("Buscar por Serie (OS)")

  df_os_filtrado = df.copy()
  if f_os_cliente and not df_os_filtrado.empty and "Cliente" in df_os_filtrado.columns:
    df_os_filtrado = df_os_filtrado[
        df_os_filtrado["Cliente"].str.contains(f_os_cliente, case=False, na=False)
    ]
  if f_os_estado != "Todos" and not df_os_filtrado.empty and "Estado" in df_os_filtrado.columns:
    df_os_filtrado = df_os_filtrado[df_os_filtrado["Estado"] == f_os_estado]
  if f_os_serie and not df_os_filtrado.empty and "Serie" in df_os_filtrado.columns:
    df_os_filtrado = df_os_filtrado[
        df_os_filtrado["Serie"].str.contains(f_os_serie, case=False, na=False)
    ]

  if df_os_filtrado.empty:
    st.info("No hay registros disponibles con los filtros actuales.")
  else:
    df_mostrar_os = df_os_filtrado.copy()
    df_mostrar_os.insert(0, "N°", range(1, len(df_mostrar_os) + 1))
    
    # Aplicar formato para resaltar filas pendientes en rojo oscuro
    df_mostrar_os_styled = df_mostrar_os.style.apply(resaltar_pendientes, axis=1)
    st.dataframe(df_mostrar_os_styled, use_container_width=True, hide_index=True)

    st.markdown("### Selecciona los equipos por su número de serie o cliente:")
    
    opciones_equipos = [
        f"Serie: {row['Serie']} | Cliente: {row['Cliente']} | Equipo: {row['Tipo de equipo']}"
        for _, row in df_os_filtrado.iterrows()
    ]
    
    seleccion_mult = st.multiselect("Equipos a incluir en la Orden de Servicio", opciones_equipos)

    filas_seleccionadas = df_os_filtrado[
        df_os_filtrado.apply(
            lambda row: f"Serie: {row['Serie']} | Cliente: {row['Cliente']} | Equipo: {row['Tipo de equipo']}" in seleccion_mult,
            axis=1
        )
    ]

    def generar_orden_servicio_multiple_pdf(seleccion_df):
      buffer = io.BytesIO()
      ...
      style_bold = ParagraphStyle('BoldText', parent=styles['Normal'], fontSize=9, leading=14, fontName='Helvetica-Bold')
      style_normal = ParagraphStyle('NormalText', parent=styles['Normal'], fontSize=9, leading=13, fontName='Helvetica')
      style_center = ParagraphStyle('CenterText', parent=styles['Normal'], fontSize=9, leading=13, fontName='Helvetica', alignment=1)
      style_header = ParagraphStyle('HeaderTxt', parent=styles['Normal'], fontSize=9, leading=13, fontName='Helvetica-Bold', textColor=colors.whitesmoke, alignment=1)

      # Configuración del logotipo con la extensión correcta (.jpeg)
      logo_imagen = Image("logo.jpeg", width=150, height=48)
      logo_imagen.hAlign = 'LEFT'

      style_address = ParagraphStyle('LogoAddress', parent=styles['Normal'], fontSize=8, leading=11, fontName='Helvetica')

      fecha_ingreso_val = seleccion_df.iloc[0].get("Fecha ingreso", datetime.today().strftime('%d/%m/%Y')) if not seleccion_df.empty else datetime.today().strftime('%d/%m/%Y')
      cliente_val = seleccion_df.iloc[0].get("Cliente", "") if not seleccion_df.empty else ""

      t_logo_estructura = Table([
          [logo_imagen],
          [Spacer(1, 6)],
          [Paragraph("<b>VENTA DE EQUIPO DE COMPUTO</b><br/>Divisoria 506, Col. Fracc. San Angel, CD Altamira,<br/>Tamaulipas, CP 89604", style_address)]
      ], colWidths=[280])
      t_logo_estructura.setStyle(TableStyle([
          ('VALIGN', (0,0), (-1,-1), 'TOP'),
      ...
      header_data = [
          [
              t_logo_estructura,
              Table([
                  [Paragraph("<b>ORDEN DE SERVICIO</b>", style_center), Paragraph("<b>ORDEN DE SERVICIO</b>", style_center)],
                  [Paragraph("<b>Fecha:</b>", style_bold), Paragraph(str(fecha_ingreso_val), style_normal)],
                  [Paragraph("<b>Teléfonos</b>", style_bold), Paragraph("(833)125-2045", style_normal)],
                  [Paragraph("<b>Oficina</b>", style_bold), Paragraph("(833)226-71-86", style_normal)],
                  [Paragraph("<b>Celular</b>", style_bold), Paragraph("(833)407-7804", style_normal)],
                  [Paragraph("<b>Correos de<br/>Atención</b>", style_bold), Paragraph("facturasdjj@gmail.com<br/>compumercadojdiaz@gmail.com", style_normal)]
              ], colWidths=[80, 160], style=[
      ...
      t_head = Table(header_data, colWidths=[280, 240])
      ...
      para_empresa_data = [
          [Paragraph("<b>PARA / EMPRESA:</b>", style_bold), Paragraph(str(cliente_val), style_normal)]
      ]
      t_para = Table(para_empresa_data, colWidths=[120, 400])
      ...
      table_content = [
          [Paragraph("<b>DESCRIPCIÓN DEL PRODUCTO O SERVICIO</b>", style_header), Paragraph("<b>CANTIDAD</b>", style_header), Paragraph("<b>DEPARTAMENTO</b>", style_header)]
      ]

      if seleccion_df.empty:
        table_content.append([Paragraph("No se seleccionaron equipos.", style_normal), Paragraph("0", style_center), Paragraph("", style_normal)])
      else:
        for _, row_data in seleccion_df.iterrows():
          desc_texto = (
          ...
          departamento_valor = str(row_data.get('Departamento', ''))
          table_content.append([
              Paragraph(desc_texto, style_normal),
              Paragraph("1", style_center),
              Paragraph(departamento_valor, style_center)
          ])

      t_prod = Table(table_content, colWidths=[320, 100, 100])
      ...
      t_firmas = Table(firma_data, colWidths=[260, 260])
    ...
    if not filas_seleccionadas.empty:
      st.success(f"Se han seleccionado {len(filas_seleccionadas)} equipo(s) para la orden de servicio.")
      pdf_os_mult_data = generar_orden_servicio_multiple_pdf(filas_seleccionadas)
      st.download_button(
          label="📥 Descargar Orden de Servicio en PDF (Seleccionados)",
          data=pdf_os_mult_data,
          file_name="orden_servicio_multiple.pdf",
          mime="application/pdf",
      )
    else:
      st.info("👆 Selecciona al menos un equipo arriba en el menú desplegable para habilitar la descarga del PDF.")
  ...
  if df_filtrado.empty:
    ...
    # Aplicar formato para resaltar filas pendientes en rojo oscuro
    df_mostrar_styled = df_mostrar.style.apply(resaltar_pendientes, axis=1)
    st.dataframe(df_mostrar_styled, use_container_width=True, hide_index=True)
  ...
  def generar_pdf(dataframe_filtrado, estado_filtro):
    ...
    elements.append(
        Paragraph(
            f"<b>Filtro de Estado aplicado:</b> {estado_filtro} | <b>Fecha de emisión:</b> {datetime.today().strftime('%d/%m/%Y')}",
            styles['Normal'],
        )
    )
    elements.append(Spacer(1, 15))
    ...
    df_pdf = dataframe_filtrado[[c for c in columnas_mostrar if c in dataframe_filtrado.columns]].copy()
    ...
    table = Table(data, colWidths=[90, 80, 70, 80, 60, 60, 180, 60])
    table.setStyle(
        TableStyle([
    ...
    if not df.empty:
      ...
      if seleccion_usuario != "-- Selecciona un equipo --":
        ...
        with st.form("form_editar"):
          ...
          e_departamento = st.text_input(
              "Departamento", value=str(df.iloc[index_a_editar].get("Departamento", ""))
          )
          ...
          if actualizar:
            ...
            df.loc[df.index[index_a_editar], "Departamento"] = e_departamento.upper()
