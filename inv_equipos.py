...
def cargar_datos():
  try:
    # Lee los datos desde la pestaña "Equipos" de tu Google Sheet (ajusta el nombre si usas otro)
    df = conn.read(worksheet="Equipos", ttl=0)
    if df is None or df.empty:
      return pd.DataFrame(columns=[
          "Cliente", "Tipo de equipo", "Serie", "Marca", "Modelo", 
          "Diagnóstico", "Observaciones", "Fecha ingreso", "Fecha salida", "Estado"
      ])
    # Asegurar que las columnas sean texto y estén limpias
...
with st.sidebar.form("form_nuevo", clear_on_submit=True):
  ...
  if submit:
    ...
    conn.update(worksheet="Equipos", data=df.astype(str))
...
if df_filtrado.empty:
  ...
  # Sustituido st.dataframe por st.data_editor para permitir edición directa
