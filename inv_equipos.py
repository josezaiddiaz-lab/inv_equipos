...
if modo_orden_servicio:
  ...
  st.markdown("Filtra los equipos, selecciónalos usando la casilla y descarga una orden de servicio consolidada.")
  ...
  if df_os_filtrado.empty:
    ...
    # Agregar columna de selección booleana antes de la tabla
    df_os_display = df_os_filtrado.copy()
    df_os_display.insert(0, "Seleccionar", False)
    
    # Mostrar editor de datos interactivo con casillas de verificación
    df_seleccionado_edit = st.data_editor(
        df_os_display,
        use_container_width=True,
        hide_index=True,
        disabled=[col for col in df_os_display.columns if col != "Seleccionar"]
    )

    # Filtrar las filas seleccionadas por el usuario
    filas_seleccionadas = df_seleccionado_edit[df_seleccionado_edit["Seleccionar"] == True]
    ...
    def generar_orden_servicio_multiple_pdf(seleccion_df):
      ...
      # Tomamos la fecha del primer registro o la de hoy
      ...
      # Construir las filas de la tabla de productos/servicios basadas en la selección múltiple
      table_content = [
    ...
    if not filas_seleccionadas.empty:
      ...
      st.info("👆 Selecciona al menos un equipo en la columna de la izquierda de la tabla para habilitar la descarga del PDF.")
