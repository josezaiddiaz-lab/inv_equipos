...
SPREADSHEET_URL = "https://docs.google.com/spreadsheets/d/1uZ_n5WaamEnCU2xXffVO-0p8DT9DXHvl/edit?gid=515113162#gid=515113162"
...
with st.expander("✏️ Editar o Eliminar Registros"):
  if not df.empty:
    opciones_filas = {f"Fila {i+1} - Cliente: {row['Cliente']} (Serie: {row['Serie']})": i for i, row in df.iterrows()}
    
    seleccion_usuario = st.selectbox(
        "Selecciona el equipo a modificar", list(opciones_filas.keys())
    )
    
    index_a_editar = opciones_filas[seleccion_usuario]
