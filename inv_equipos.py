...
if modo_orden_servicio:
  ...
  if df_os_filtrado.empty:
    ...
    def generar_orden_servicio_multiple_pdf(seleccion_df):
      ...
      # Estilos específicos para el logotipo estructurado por filas para evitar solapamientos
      style_logo_top = ParagraphStyle('LogoTop', parent=styles['Normal'], fontSize=10, fontName='Helvetica-Oblique', textColor=colors.black)
      style_logo_main = ParagraphStyle('LogoMain', parent=styles['Normal'], fontSize=22, fontName='Helvetica-Bold', textColor=colors.HexColor('#1f3a93'))
      style_logo_sub = ParagraphStyle('LogoSub', parent=styles['Normal'], fontSize=12, fontName='Helvetica-Bold', textColor=colors.black)
      ...
      # Tabla interna para separar cada línea del logotipo con mayor espacio antes de la dirección
      t_logo_estructura = Table([
          [Paragraph("Calidad y Servicio", style_logo_top)],
          [Paragraph("GCM", style_logo_main)],
          [Paragraph("compumercado", style_logo_sub)],
          [Spacer(1, 12)],  # Espacio aumentado para evitar el encime
