...
with st.sidebar.form("form_nuevo", clear_on_submit=True):
  ...
  if submit:
    nuevo_registro = pd.DataFrame({
        "Cliente": [
            c_cliente.upper()
        ],  # <--- Solo el cliente se fuerza a mayúsculas
...
# Aplicar filtros (Si buscas por cliente, se convierte a mayúsculas para que coincida perfectamente)
df_filtrado = df.copy()
if filtro_cliente:
    ...
    with st.form("form_editar"):
      ...
      if actualizar:
        df.loc[index_a_editar] = [
            e_cliente.upper(),  # <--- Solo el cliente se guarda en mayúsculas al actualizar
            e_tipo,
            e_serie,
            e_marca,
            e_modelo,
